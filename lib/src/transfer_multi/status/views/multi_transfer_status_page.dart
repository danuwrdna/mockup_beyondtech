...
import 'package:bpay_mobile/src/transfer_multi/status/views/multi_transfer_success_page.dart';
...
class MultiTransferStatusPage extends StatefulWidget {
  const MultiTransferStatusPage({
    super.key,
    this.refId,
  ...
  @override
  State<MultiTransferStatusPage> createState() =>
      _MultiTransferStatusPageState();
}

class _MultiTransferStatusPageState extends State<MultiTransferStatusPage> {
  ...
  void startWaitingTransaction(BuildContext context) async {
    ...
    if (context.mounted && widget.refId != null) {
      context.read<MultiTransferStatusBloc>().add(
          MultiTransferStatusEvent.getTransactionStatus(widget.refId ?? ""));
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocListener<MultiTransferStatusBloc, MultiTransferStatusState>(
            ...
            success: (data) {
              if (data.data?[0].statusTransaction ==
                ...
                Navigator.pushAndRemoveUntil(
                  context,
                  MaterialPageRoute(
                    builder: (context) => MultiTransferSuccessPage(
                                ...
                                children: [
                                  Row(
                                    children: [
                                      ...
                                      Text(
                                        convertToIdr(
                                          widget.destinations[index].amount,
                                          0,
                                        ),
                                        style: TextStyle(
                                          fontSize: 12.sp,
